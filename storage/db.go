...
func (db *DB) createTables() error {
	...
	BEGIN 
		...
		END IF;
		IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='links' AND column_name='deep_link') THEN
			ALTER TABLE links ADD COLUMN deep_link TEXT;
		END IF;
...
func (db *DB) CreateLink(link *models.Link) error {
	query := `
		INSERT INTO links (id, universal_link, deep_link, ios_store, android_store, title, description, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
	`
	
	_, err := db.conn.Exec(query, link.ID, link.UniversalLink, link.DeepLink, link.IOSStore, link.AndroidStore,
...
func (db *DB) GetLink(id string) (*models.Link, error) {
	query := `
		SELECT id, universal_link, deep_link, ios_store, android_store, title, description, created_at, updated_at, click_count
		FROM links WHERE id = $1
	`
	
	link := &models.Link{}
	err := db.conn.QueryRow(query, id).Scan(
		&link.ID, &link.UniversalLink, &link.DeepLink, &link.IOSStore, &link.AndroidStore,
